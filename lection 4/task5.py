...
def josephus_task(num_people, kill_num):
    ...
    # Решаем от обратного: к индексу выжившего прибавляем количество пропущенных и убитых (kill_num)
    # Во избежание выхода за длину списка считаем остаток от деления индекса на длину списка (количество людей)
    # Каждая итерация цикла по сути является воскрешением :D

    survivor = 0  # Индекс последнего выжившего всегда 0
    for kill in range(1, num_people):  # убийств всегда num_people - 1
        survivor = (survivor + kill_num) % (kill + 1)
    survivor += 1  # Для получения итогового значения прибавляем 1 к индексу
    return survivor
